...
if ($latest) {
    ...
    if (is_numeric($issueNumStr)) {
        ...
        if ($matches) {
            ...
            $betIssue = $issueNumStr; // Fallback
        }
    }

    // 1. Closure Summary (Verification)
    if ($countdown > 10 && $countdown <= 20) {
        ...
        $checkStmt->execute(["%上期下注核对%"]);

        if (!$checkStmt->fetch()) {
            // Get all user bets for this issue
            $stmt = $db->prepare("SELECT u.nickname, u.username, b.play_type, b.bet_amount
                                 FROM bets b
                                 JOIN users u ON b.user_id = u.id
                                 WHERE b.issue_no = ? AND u.is_robot = 0
                                 ORDER BY b.id ASC");
            $stmt->execute([$betIssue]);
            ...
            foreach($userAgg as $name => $bets) {
                $summaryLines[] = "{$name}: " . implode(", ", $bets);
            }
