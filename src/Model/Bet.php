...
class Bet {
    public static function place($userId, $issueNo, $playType, $amount, $oddsType = 'low', $inTransaction = false) {
        ...
        try {
            ...
            if ($balanceBefore < $amount) throw new Exception("积分余额不足");

            // Get odds - Robust matching
            ...
            // If it's a number bet (0-27) and not explicitly configured, default to 12.00
            if ($odds === false && is_numeric($playType)) {
                $num = (int)$playType;
                if ($num >= 0 && $num <= 27) $odds = 12.00;
            }

            if ($odds === false) throw new Exception("无效的玩法标识: " . $playType);
            ...
            $stmt->execute(array($userId, $issueNo, $playType, $oddsType, $amount, (float)$odds));
